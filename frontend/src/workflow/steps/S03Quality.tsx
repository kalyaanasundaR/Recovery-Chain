...
export default function S03Quality({ ctx, next, setAction }: StepProps) {
    ...
    const verdict = score === null ? { w: '—', t: 'gray' }
        : score >= 85 ? { w: 'Good — ready to use', t: 'green' }
            : score >= 60 ? { w: 'Usable, with a few gaps', t: 'amber' }
                : { w: 'Needs a closer look', t: 'red' };

    // completeness
    const totalCells = rows * cols.length;
    const missingCells = cols.reduce((s, c) => s + (c.missing_count || 0), 0);
    const filledPct = totalCells > 0 ? Math.round(((totalCells - missingCells) / totalCells) * 100) : 100;
    const gappy = cols.filter(c => (c.missing_count || 0) > 0)
    ...
    const coveredRoles = new Set(
        detected.map(s => (s.canonical_field && s.canonical_field !== 'UNKNOWN') ? ROLE_OF[s.canonical_field] : null).filter(Boolean));
    ...
    return (
        <div className="max-w-2xl">
            ...
            <div className="mt-8">
                <BigStat label="Data health score" value={score === null ? '—' : `${score} / 100`}
                    tone={verdict.t as any} sub={verdict.w} />
            </div>

            {/* 1. is it filled in */}
            <h2 className="mt-8 text-sm font-semibold uppercase tracking-wide text-[--faint]">Is it filled in?</h2>
                    ...
                    <Line good={false}>
                        ...
                        {gappy.slice(0, 4).map(c => (
                            <span key={c.column_name} className="whitespace-nowrap">
                                ...
                                ({Math.round((c.missing_rate || 0) * 100)}% empty){gappy.indexOf(c) < Math.min(gappy.length, 4) - 1 ? ', ' : ''}
                            </span>
                        ))}
                        {gappy.length > 4 && <> +{gappy.length - 4} more</>}.
                        {' '}Blank cells are filled with a safe default before scoring.
                    </Line>
                ) : (
            ...
            <h2 className="mt-8 text-sm font-semibold uppercase tracking-wide text-[--faint]">Is anything important missing?</h2>
            <ul className="mt-1 divide-y divide-[--line] rounded-xl border border-[--line] bg-[--panel] px-4">
                {missingRoles.length === 0 ? (
                    <Line good>All four needed columns were found: customer, amount, date, result.</Line>
                ) : (
                    <Line good={false}>
                        We haven’t clearly found: <b>{missingRoles.join(', ')}</b>. You point at the right
                        column in step&nbsp;4 — it won’t be guessed.
                    </Line>
                )}
                {constants.length > 0 ? (
                    <Line good={false}>
                        Same value in every row (tells the model nothing, so it’s dropped):{' '}
                        {constants.map(c => <span key={c.column_name} className="font-mono">{c.column_name} </span>)}
                    </Line>
                ) : (
            ...
            <h2 className="mt-8 text-sm font-semibold uppercase tracking-wide text-[--faint]">Could anything “leak” the answer?</h2>
            <ul className="mt-1 divide-y divide-[--line] rounded-xl border border-[--line] bg-[--panel] px-4">
                {leakage.length === 0 ? (
                    <Line good>
                        Nothing found. No column here reveals whether the money came back before the model
                        would actually know.
                    </Line>
                ) : (
                    leakage.map((l: any) => (
                        <Line key={l.column} good={false}>
                            <span className="font-mono">{l.column}</span> is filled in only <i>after</i> the
                            outcome is known. Letting the model see it would make its score fake — so it’s
                ...
                <div className="mt-3 overflow-x-auto rounded-xl border border-[--line]">
                    <table className="w-full text-xs">
                        <thead className="bg-white/[0.03] text-left text-[--muted]">
                            <tr>
                                <th className="px-3 py-2">Column</th><th className="px-3 py-2">Kind</th>
                                <th className="px-3 py-2">Different values</th><th className="px-3 py-2">Blank</th>
                            </tr>
                        </thead>
                            ...
                            {cols.map(c => (
                                <tr key={c.column_name}>
                                    ...
                                    <td className="px-3 py-1.5 text-[--muted]">
                                        {String(c.dtype).includes('int') || String(c.dtype).includes('float') ? 'number'
                                            : String(c.dtype).includes('date') ? 'date' : 'text'}
                                    </td>
                                    <td className="px-3 py-1.5 tabular">{c.unique_count}</td>
                                    <td className="px-3 py-1.5 tabular">{Math.round((c.missing_rate || 0) * 100)}%</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            <div className="mt-8">
                <Note>
                    This checks the whole file. The row-by-row check — bad amounts, unreadable dates, rows
                    that were already paid — runs when cases are built in step&nbsp;6, and you’ll see those
                    counts there.
                </Note>
            </div>
        </div>
