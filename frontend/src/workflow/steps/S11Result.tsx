...
export default function S11Result({ ctx, patch, setAction }: StepProps) {
    ...
    useEffect(() => {
        ...
        if (snap.execution_record?.status === 'COMPLETED_SIMULATED' && !snap.outcome) {
            ...
            verifyCase(ctx.activeCaseId!, snap.execution_record.execution_id)
                .then(refresh).catch(() => { }).finally(() => setVerifying(false));
        }
    }, [snap]);

    if (loading || !snap || verifying) {
        return err ? <ErrorNote onRetry={refresh}>{err}</ErrorNote> : <Spinner label={verifying ? 'Verifying the outcome…' : 'Loading result…'} />;
    }
    ...
    return (
        <div className="max-w-2xl">
            <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[--faint]">Recovery result</div>
            ...
            {o ? (
                <>
                    ...
                    <div className="mt-8">
                        <Note>
                            Verified against the sandbox settlement source ({o.verification_source}). The outcome is
                            checked independently — running an action does not by itself mean money was recovered.
                        </Note>
                    </div>
                </>
            ) : (
                <>
                    <div className="mt-3"><BigStat label={snap.customer_id} value="No recovery attempted" tone="gray" /></div>
                    <div className="mt-6 rounded-2xl border border-[--line] bg-[--panel] px-5 py-4 text-sm text-[--muted]">
                        {snap.policy_decision?.reason || 'The policy check did not approve an action, so nothing was executed or verified.'}
                    </div>
                </>
            )}
