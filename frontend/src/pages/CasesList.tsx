...
export default function CasesList() {
    ...
    const load = () => {
        ...
        listCases().then(d => setCases(d || [])).catch(e => setErr(e.message)).finally(() => setLoading(false));
    };
    useEffect(load, []);
    ...
    const rows = useMemo(() => cases.filter(c => {
        ...
        if (filter === 'running') return !c.outcome_status && c.policy_status !== 'ESCALATE';
        return true;
    }), [cases, filter, q]);
    ...
    return (
        <div className="fade-in space-y-6">
            <div>
                ...
                <p className="mt-1 text-slate-400">One row per recovery case. Chance-back is the estimate; Recovered is what the sandbox settled.</p>
            </div>
                                ...
                                {rows.map(c => {
                                    ...
                                    return (
                                        <tr key={c.case_id} className="hover:bg-slate-800/40">
                                            ...
                                            <td className="py-3 pr-4 tabular-nums text-slate-300">{pct(c.recovery_probability)}</td>
                                            <td className="py-3 pr-4 text-slate-300">
                                                {c.recommended_action ? (ACTION[c.recommended_action] || c.recommended_action) : '—'}
                                            </td>
                                            ...
                                            <td className="py-3 tabular-nums text-emerald-300">
                                                {c.actual_amount_recovered ? money(c.actual_amount_recovered) : <span className="text-slate-600">—</span>}
                                            </td>
                                        </tr>
                                    );
                                })}
