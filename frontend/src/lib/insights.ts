...
function groupBy(cases: any[], field: string): Group[] {
    const map = new Map<string, { count: number; atRisk: number; recovered: number; probSum: number; probN: number }>();
    for (const c of cases) {
        ...
        if (c.recovery_probability != null) { g.probSum += num(c.recovery_probability); g.probN++; }
        map.set(k, g);
    }
    return [...map.entries()]
        .map(([key, g]) => ({
            key, count: g.count, atRisk: g.atRisk, recovered: g.recovered,
...
export function computeInsights(cases: any[]) {
    ...
    return {
        ...
        outcomeCounts: {
            ...
            PARTIALLY_RECOVERED: cases.filter(c => c.outcome_status === 'PARTIALLY_RECOVERED').length,
            NOT_RECOVERED: cases.filter(c => c.outcome_status === 'NOT_RECOVERED').length,
            PENDING_VERIFICATION: cases.filter(c => c.outcome_status === 'PENDING_VERIFICATION').length,
