...
from domain.models import RecoveryCase, RevenueEvent, RiskAssessment, RootCauseDiagnosis, RecoveryPrediction, ActionRecommendation, PolicyDecision, ExecutionRecord, RecoveryOutcome
...
class InMemoryCaseRepository(ICaseRepository):
    ...
    def get_active_case_for_customer(self, customer_id: str, risk_category: str, reference_id: Optional[str]) -> Optional[RecoveryCase]:
        ...
        for case in self.cases.values():
            if case.customer_id == customer_id and case.risk_category == risk_category and case.reference_id == reference_id:
...
class SqlAlchemyAuditRecorder(IAuditRecorder):
    ...
    def get_execution_record_by_key(self, key: str) -> dict:
        record = self.db.query(IdempotencyRecord).filter(IdempotencyRecord.idempotency_key == key).first()
...
class SqlAlchemyCaseRepository(ICaseRepository):
    ...
    def save(self, case: RecoveryCase) -> None:
        ...
        if not db_case:
            db_case = CaseModel(
                ...
                expected_recoverable_value=case.expected_recoverable_value.amount if case.expected_recoverable_value else None,
                currency=case.amount_at_risk.currency,
                current_state=case.current_state,
                risk_assessment=case.risk_assessment.model_dump(mode="json") if case.risk_assessment else None,
                ...
                recommendation=case.recommendation.model_dump(mode="json") if case.recommendation else None,
                policy_decision=case.policy_decision.model_dump(mode="json") if case.policy_decision else None,
                execution_record=case.execution_record.model_dump(mode="json") if case.execution_record else None,
            ...
            db_case.expected_recoverable_value = case.expected_recoverable_value.amount if case.expected_recoverable_value else None
    ...
    def get_by_id(self, case_id: str) -> Optional[RecoveryCase]:
        ...
        case = RecoveryCase(
            ...
            expected_recoverable_value=Money(amount=db_case.expected_recoverable_value, currency=db_case.currency) if db_case.expected_recoverable_value is not None else None,
            current_state=db_case.current_state,
            risk_assessment=RiskAssessment(**db_case.risk_assessment) if db_case.risk_assessment else None,
            ...
            recommendation=ActionRecommendation(**db_case.recommendation) if db_case.recommendation else None,
            policy_decision=PolicyDecision(**db_case.policy_decision) if db_case.policy_decision else None,
            execution_record=ExecutionRecord(**db_case.execution_record) if db_case.execution_record else None,
            ...
            linked_events=[
                RevenueEvent(
                    ...
                    timestamp=e.timestamp.replace(tzinfo=timezone.utc) if e.timestamp.tzinfo is None else e.timestamp,
    ...
    def get_event_by_external_id(self, external_system: str, external_event_id: str) -> Optional[RevenueEvent]:
        e = self.db.query(EventModel).filter(
    ...
    def get_active_case_for_customer(self, customer_id: str, risk_category: str, reference_id: Optional[str]) -> Optional[RecoveryCase]:
        ...
        db_case = self.db.query(CaseModel).filter(
