...
from domain.models import RecoveryCase, RevenueEvent, RiskCategory, Money, RiskLevel, RiskAssessment
from application.risk_detector import DeterministicRiskDetector

def create_mock_case(risk_category: RiskCategory, amounts: list[float], age_hours: list[float]) -> RecoveryCase:
    now = datetime.now(timezone.utc)
    events = []
    for i, (amount, age) in enumerate(zip(amounts, age_hours)):
...
def test_J_thresholds(detector):
    ...
    assert detector.assess_risk(create_mock_case(RiskCategory.FAILED_PAYMENT, [1.0], [1.0])).risk_level == RiskLevel.LOW
    assert detector.assess_risk(create_mock_case(RiskCategory.FAILED_PAYMENT, [500.0, 500.0], [2.0, 1.0])).risk_level == RiskLevel.MEDIUM
    assert detector.assess_risk(create_mock_case(RiskCategory.FAILED_PAYMENT, [1000.0, 1000.0, 1000.0, 1000.0], [4.0, 3.0, 2.0, 1.0])).risk_level == RiskLevel.HIGH
    assert detector.assess_risk(create_mock_case(RiskCategory.FAILED_PAYMENT, [5000.0] * 5, [5.0, 4.0, 3.0, 2.0, 1.0])).risk_level == RiskLevel.CRITICAL
