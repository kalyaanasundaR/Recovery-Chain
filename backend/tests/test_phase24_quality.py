...
def test_common_separators_mapping():
    ...
    for var in variations:
        ...
        assert m["canonical_field"] in ["ACCOUNT_ID", "ENTITY_ID", "CUSTOMER_ID"], f"Failed for {var}"
...
def test_data_quality_score_deterministic():
    ...
    dq_missing = DatasetProfiler.calculate_data_quality_score(df, cols_profile, mapped_schema_missing, [], {})
    ...
    cols_profile_imb = [{"column_name": c, "is_constant": c in ["amount", "date"]} for c in df_imb.columns]
    
    dq_imb = DatasetProfiler.calculate_data_quality_score(df_imb, cols_profile_imb, mapped_schema, [], {})
