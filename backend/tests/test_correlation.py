import pytest
from domain.models import RevenueEvent, RiskCategory, Money, CaseState
...
class InMemoryAuditRecorder(IAuditRecorder):
    ...
    def log_transition(self, case_id: str, from_state: str, to_state: str, evidence: dict) -> None:
        self.logs.append({"case_id": case_id, "from": from_state, "to": to_state, "evidence": evidence})
...
def create_event(ext_sys: str, ext_id: str, cust_id: str, risk: RiskCategory, ref_id: str, amount: float):
    return RevenueEvent(
...
def test_B_diff_system_same_event_id_distinct(engine):
    evt1 = create_event("stripe", "evt_001", "cust_1", RiskCategory.FAILED_PAYMENT, "tx_123", 100.0)
    evt2 = create_event("razorpay", "evt_001", "cust_1", RiskCategory.FAILED_PAYMENT, "tx_123", 100.0)
...
def test_E_same_customer_diff_invoice_ids_separate_cases(engine):
    evt1 = create_event("billing", "evt_001", "cust_1", RiskCategory.OVERDUE_INVOICE, "inv_A", 100.0)
    evt2 = create_event("billing", "evt_002", "cust_1", RiskCategory.OVERDUE_INVOICE, "inv_B", 200.0)
    c1, _ = engine.ingest_normalized_event(evt1)
    c2, is_new = engine.ingest_normalized_event(evt2)
    assert is_new is True
    assert c1.case_id != c2.case_id
...
def test_F_same_underlying_payment_multiple_events_one_master_case(engine):
    evt1 = create_event("stripe", "evt_initial", "cust_1", RiskCategory.FAILED_PAYMENT, "tx_master", 100.0)
    evt2 = create_event("stripe", "evt_retry1", "cust_1", RiskCategory.FAILED_PAYMENT, "tx_master", 100.0)
    evt3 = create_event("stripe", "evt_retry2", "cust_1", RiskCategory.FAILED_PAYMENT, "tx_master", 100.0)
    c1, _ = engine.ingest_normalized_event(evt1)
...
def test_H_amount_at_risk_does_not_double_count(engine):
    # Base failure
    evt1 = create_event("stripe", "evt_fail_1", "cust_1", RiskCategory.FAILED_PAYMENT, "tx_123", 100.0)
    c1, _ = engine.ingest_normalized_event(evt1)
    ...
    evt2 = create_event("stripe", "evt_fail_2", "cust_1", RiskCategory.FAILED_PAYMENT, "tx_123", 105.0)
    c2, _ = engine.ingest_normalized_event(evt2)
