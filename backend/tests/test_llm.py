...
from evaluation.runner import EvaluationRunner, AblationConfig
...
def test_real_llm_unavailable_fallback(session):
    runner = EvaluationRunner(session, ablation=AblationConfig(experiment_mode="REAL_LLM_FULL_REASONING"))
    scenario = [s for s in SCENARIOS if s.scenario_id == "FP_1_STANDARD"][0]
    result = runner.run_scenario(scenario)
