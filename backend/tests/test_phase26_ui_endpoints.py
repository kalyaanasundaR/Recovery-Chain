...
def test_dataset_predict_endpoint_and_state(synthetic_bank_datasets):
    ...
    # Check Prediction
    # Complete features
    pred_res = client.post(f"/datasets/{ds_id}/predict", json={"canonical_features": {"AMOUNT": 10.5, "TIMESTAMP": "2023-01-01", "ACCOUNT_ID": "A1"}})
    assert pred_res.status_code == 200
    assert "probability" in pred_res.json()
    
    # Missing fields
    pred_bad = client.post(f"/datasets/{ds_id}/predict", json={"canonical_features": {"AMOUNT": 10.5}})
    assert pred_bad.status_code == 400
    assert "Incomplete feature set. Missing original features" in pred_bad.json()["detail"]
    
    # Isolation (Bad ID)
    pred_iso = client.post("/datasets/invalid_id/predict", json={"canonical_features": {"AMOUNT": 10.5}})
