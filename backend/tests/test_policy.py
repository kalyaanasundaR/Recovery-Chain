...
from domain.models import (
    RecoveryCase, RevenueEvent, RiskCategory, Money, 
...
@pytest.fixture
def policy_engine():
    return DeterministicPolicyEngine(MerchantPolicy(
...
def test_B_retry_limit_exceeded(policy_engine):
    ...
    old_events = [
        RevenueEvent(event_id=f"evt_{i}", customer_id="c", risk_category=RiskCategory.FAILED_PAYMENT, external_system="s", external_event_id="e", reference_id="r", amount=Money(amount=100.0), timestamp=now - timedelta(days=5), raw_payload={})
        for i in range(3)
    ]
    case = create_mock_case_with_recommendation(ActionType.RETRY_PAYMENT, amount=100.0, events=old_events)
    decision = policy_engine.evaluate(case)
...
def test_C_retry_cooling_period_active(policy_engine):
    now = datetime.now(timezone.utc)
    recent_events = [
        RevenueEvent(event_id="evt_1", customer_id="c", risk_category=RiskCategory.FAILED_PAYMENT, external_system="s", external_event_id="e", reference_id="r", amount=Money(amount=100.0), timestamp=now - timedelta(hours=2), raw_payload={})
    ]
    case = create_mock_case_with_recommendation(ActionType.RETRY_PAYMENT, amount=100.0, events=recent_events)
    decision = policy_engine.evaluate(case)
    assert decision.status == PolicyDecisionStatus.WAIT
    assert any(r.rule_name == "PaymentRetryCooldown" and not r.passed for r in decision.failed_rules)

def test_D_communication_frequency(policy_engine):
    now = datetime.now(timezone.utc)
    recent_events = [
        RevenueEvent(event_id=f"evt_{i}", customer_id="c", risk_category=RiskCategory.FAILED_PAYMENT, external_system="s", external_event_id="e", reference_id="r", amount=Money(amount=100.0), timestamp=now - timedelta(hours=2), raw_payload={})
        for i in range(3) # Max is 2
    ]
    case = create_mock_case_with_recommendation(ActionType.SEND_PAYMENT_REMINDER, amount=100.0, events=recent_events)
    decision = policy_engine.evaluate(case)
    assert decision.status == PolicyDecisionStatus.WAIT
    assert any(r.rule_name == "CommunicationMaxMessages" and not r.passed for r in decision.failed_rules)

def test_F_financial_amount_escalate(policy_engine):
    case = create_mock_case_with_recommendation(ActionType.RETRY_PAYMENT, amount=10000.0) # Limit is 5000
    decision = policy_engine.evaluate(case)
    assert decision.status == PolicyDecisionStatus.ESCALATE

def test_H_insufficient_evidence(policy_engine):
    case = RecoveryCase(
        case_id="case_1", customer_id="c", risk_category=RiskCategory.FAILED_PAYMENT,
        reference_id="r", amount_at_risk=Money(amount=100.0)
    )
    decision = policy_engine.evaluate(case)
    assert decision.status == PolicyDecisionStatus.ESCALATE
