...
def bad_dataset(tmp_path):
    ...
    # Add the bad rows
    bad_rows = [
        {"account_no": "CUST2", "txn_amount": "invalid", "transaction_date": "2023-01-02", "payment_failed": "1"},
        {"account_no": "CUST3", "txn_amount": -5.0, "transaction_date": "2023-01-03", "payment_failed": "1"},
        {"account_no": "CUST4", "txn_amount": 100.0, "transaction_date": "bad_date", "payment_failed": "1"},
        {"account_no": "", "txn_amount": 50.0, "transaction_date": "2023-01-05", "payment_failed": "1"},
        {"account_no": "CUST6", "txn_amount": 20.0, "transaction_date": "2023-01-06", "payment_failed": "unknown"},
    ]
...
def test_generate_cases_robustness(bad_dataset):
    ...
    with open(bad_dataset, "rb") as f:
        res = client.post("/datasets/upload", files={"file": (f"ui_test_{uuid.uuid4().hex[:8]}.csv", f, "text/csv")})
    assert res.status_code == 200, f"{res.text} | Suitability: {client.get(f'/datasets/{ds_id}').json().get('training_suitability')}"
    ...
    assert data["cases_generated"] == 2, f"Expected 2 cases from valid rows, got {data['cases_generated']}."
