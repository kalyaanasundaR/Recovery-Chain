...
from application.dataset_intelligence import DatasetClassification, CanonicalField
...
def synthetic_datasets(tmp_path_factory):
    ...
    df_b = pd.DataFrame({
        ...
        "settlement_result": ["success", "failure", "success", "failure", "success", "failure", "success", "failure", "success", "failure"]
    })
    ...
    return {
        "A": path_a, "B": path_b, "C": path_c, "D": path_d, "E": path_e
    }
...
def test_workflow_dataset_a(synthetic_datasets):
    ...
    assert ds["training_suitability"].get("overall_classification", ds["training_suitability"].get("readiness_status")) == DatasetClassification.ML_TRAINING_READY.value
...
def test_workflow_dataset_d_insufficient(synthetic_datasets):
    ...
    assert ds["training_suitability"].get("overall_classification", ds["training_suitability"].get("readiness_status")) == DatasetClassification.PARTIALLY_USABLE.value

def test_workflow_dataset_e_leakage(synthetic_datasets):
    ds = upload_and_analyze(synthetic_datasets["E"])
    assert ds["training_suitability"].get("overall_classification", ds["training_suitability"].get("readiness_status")) == DatasetClassification.ANALYSIS_READY.value
