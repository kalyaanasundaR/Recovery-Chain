...
def test_dataset_artifact_isolation(test_dataset, mock_spec, tmp_path):
    ...
    predictor = MLPaymentFailurePredictor(dataset_id="ds_wrong_one", registry_dir=str(tmp_path), legacy_model_path="missing", legacy_features_path="missing")
...
def test_preprocessing_reused(test_dataset, mock_spec, tmp_path):
    ...
    is_calibrated_pipeline = isinstance(model, CalibratedClassifierCV) and isinstance(model.estimator, Pipeline)
