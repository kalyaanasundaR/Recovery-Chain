...
from domain.models import RecoveryCase, RevenueEvent, RiskCategory, Money, CaseState
...
def _case(risk_category, failure_code):
    ev = RevenueEvent(
        event_id="evt_x", customer_id="C1", risk_category=risk_category,
        external_system="test", external_event_id="e1", reference_id="r1",
        amount=Money(amount=100), timestamp=pd.Timestamp("2026-01-01", tz="UTC").to_pydatetime(),
        raw_payload={"failure_code": failure_code},
    )
    return RecoveryCase(case_id="case_x", customer_id="C1", risk_category=risk_category,
                        reference_id="r1", linked_events=[ev], amount_at_risk=Money(amount=100),
                        current_state=CaseState.OPEN)


# ---------------------------------------------------------------- M1d
@pytest.mark.fast
@pytest.mark.parametrize("cat, code, expected", [
    ...
    (RiskCategory.FAILED_PAYMENT, "some_unmapped_gateway_code", "MANDATE_FAILURE"),  # generic fallback
...
def test_ml_training_reads_xlsx(tmp_path):
    from application.ml_training import _read_any, MLTrainingEngine
    rows = []
    for i in range(60):
        failed = i % 2
        rows.append({"acct": f"a{i}", "amt": 10.0 + i, "day": f"2026-01-{1 + i % 27:02d}",
    ...
    spec = {
        "dataset_id": "ds_xlsx_test", "prediction_problem": "payment-failure-risk",
        "target_column": "res", "feature_columns": ["amt", "reason", "tier"],
        "excluded_columns": ["acct", "day", "res"],
        "temporal_split": {"strategy": "TEMPORAL_CHRONOLOGICAL", "split_column": "day"},
    }
    meta = MLTrainingEngine(spec, str(xlsx), str(tmp_path / "reg"),
                            min_roc_auc=0.0, min_test_rows=0).train_and_evaluate()
...
def test_generate_cases_risk_category_payload_and_export():
    ...
    for i in range(60):
        ...
        rows.append({
            ...
            "event_timestamp": pd.Timestamp("2026-02-01") + pd.Timedelta(hours=i),  # real Timestamp
            "amount": 100.0 + i,
    ...
    r = client.post("/datasets/upload",
                    files={"file": (f"rt_{uuid.uuid4().hex[:6]}.xlsx", buf,
    ...
    client.post(f"/datasets/{ds}/mapping", json={"mappings": [
        {"original_column": s["original_column"], "canonical_field": s["canonical_field"],
         "action": "confirm" if s["canonical_field"] != "UNKNOWN" else "unused"} for s in sigs]})
