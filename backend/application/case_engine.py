...
from domain.models import RevenueEvent, RecoveryCase, RiskCategory, Money, CaseState
    ...
    def ingest_normalized_event(self, event: RevenueEvent) -> Tuple[RecoveryCase, bool]:
        ...
        if active_case:
            ...
            self.audit_recorder.log_transition(
                ...
                from_state=case.current_state, to_state=case.current_state,
                evidence={"action": "event_attached", "event_id": event.event_id, "reference_id": event.reference_id}
            )
        else:
            is_new_case = True
            case = RecoveryCase(
            ...
            self.audit_recorder.log_transition(
                case_id=case.case_id,
                from_state=CaseState.DETECTED, to_state=CaseState.OPEN,
                evidence={"action": "case_created", "trigger_event": event.event_id, "reference_id": event.reference_id}
            )
