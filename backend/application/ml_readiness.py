    ...
    def analyze_readiness(ds, file_path: str) -> Dict[str, Any]:
        ...
        if target_column:
            ...
            if "late" in t_lower or "delinquent" in t_lower:
                ...
                target_definition = "Predicts risk of an invoice/payment settling late (but eventually settling)."
        ...
        for c in profile:
            ...
            if c_schema:
                if c_schema["canonical_field"] in [CanonicalField.ENTITY_ID.value, CanonicalField.ACCOUNT_ID.value, CanonicalField.CUSTOMER_ID.value]:
                    entity_col = c_name
                elif c_schema["canonical_field"] in [CanonicalField.TIMESTAMP.value, CanonicalField.SETTLEMENT_DATE.value]:
                    time_col = c_name
            ...
            if c_name in leakage_map:
                excluded_columns.append(c_name)
                exclusion_reasons[c_name] = f"POST_OUTCOME / TEMPORALLY_AMBIGUOUS: {leakage_map[c_name]}"
                continue
            ...
            if c_schema and c_schema["canonical_field"] in [CanonicalField.ENTITY_ID.value, CanonicalField.ACCOUNT_ID.value, CanonicalField.CUSTOMER_ID.value]:
                excluded_columns.append(c_name)
        ...
        if time_col and entity_col:
            ...
            entity_strategy = "Group by ENTITY_ID to prevent train/test leakage of future entity states."
        elif time_col:
            temporal_split = {
        ...
        if not target_column:
            ...
            metrics = ["ROC-AUC", "PR-AUC", "precision", "recall"] if is_imbalanced else ["accuracy", "F1"]
            if is_imbalanced:
                warnings.append("Target is highly imbalanced. Avoid raw accuracy as primary metric.")
                
            if not feature_columns:
                status = "INSUFFICIENT_DATA"
                warnings.append("No safe predictive features remain after leakage and identifier exclusion.")
            elif not time_col:
                status = "LEAKAGE_RISK"
                warnings.append("No temporal column detected; historical cutoff enforcement impossible.")
            else:
        ...
        canonical_mapping = {s["original_column"]: s["canonical_field"] for s in schema if s["original_column"] in feature_columns}
