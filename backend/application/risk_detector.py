...
class DeterministicRiskDetector:
    ...
    def assess_risk(self, case: RecoveryCase, assessment_time: Optional[datetime] = None) -> RiskAssessment:
        ...
        # Bounded amount score: 1 - e^(-amount / 500)
        # Gives a smooth curve 0 to 1 where amount=500 -> 0.63, amount=2000 -> 0.98
        amount_score = 1 - math.exp(-amount / 500.0) if amount > 0 else 0
        
        if case.risk_category == RiskCategory.FAILED_PAYMENT:
            # Score influenced heavily by amount and number of retries
            retry_score = min(event_count / 5.0, 1.0)
            score = amount_score * 0.6 + retry_score * 0.4
            
        elif case.risk_category == RiskCategory.CHECKOUT_ABANDONMENT:
            # Abandonment is generally lower structural risk than a failed settled payment
            score = amount_score * 0.5
            
        elif case.risk_category == RiskCategory.FAILED_SUBSCRIPTION:
            # Subscriptions have high churn risk after multiple failures
            churn_risk = min(event_count / 3.0, 1.0)
            score = amount_score * 0.5 + churn_risk * 0.5
