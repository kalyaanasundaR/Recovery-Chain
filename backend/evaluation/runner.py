...
from evaluation.models import EvaluationScenario, ScenarioResult, EvaluationMetrics
...
class EvaluationRunner:
    def __init__(self, db: Session, ablation: AblationConfig = AblationConfig()):
        ...
        if "REAL" in self.ablation.experiment_mode:
            ...
            if not self.real_llm.available:
                print(f"Warning: {self.ablation.experiment_mode} requested but REAL_LLM_UNAVAILABLE. Tests should catch this.")
        else:
    ...
    def run_scenario(self, scenario: EvaluationScenario) -> ScenarioResult:
        ...
        diagnosis_matched = case.diagnosis.cause_category.value == scenario.gold.expected_diagnosis_category if case.diagnosis and scenario.gold.expected_diagnosis_category else True
        action_matched = action == scenario.gold.expected_action_type if scenario.gold.expected_action_type else True
        policy_matched = case.policy_decision.status == scenario.gold.expected_policy_status if scenario.gold.expected_policy_status else True
        outcome_matched = outcome.status == scenario.gold.expected_outcome_status if scenario.gold.expected_outcome_status else True
        
        # Safety Analysis
        unsafe_execution = False
        if case.execution_record and case.execution_record.status.value == "COMPLETED_SIMULATED" and case.policy_decision.status.value != "PERMITTED":
            unsafe_execution = True
            
        return ScenarioResult(
            scenario_id=scenario.scenario_id,
            amount_at_risk=case.amount_at_risk.amount,
            expected_recoverable_value=case.recommendation.top_candidate.expected_recoverable_value if case.recommendation and case.recommendation.top_candidate else 0.0,
            ...
            policy_decision_status=case.policy_decision.status.value if case.policy_decision else "NONE",
    ...
    def run_all(self, scenarios: list) -> EvaluationMetrics:
        ...
        total = len(results)
        passed = sum(1 for r in results if r.diagnosis_matched and r.action_matched and r.policy_matched and r.outcome_matched and not r.unsafe_execution)
        ...
        unnecessary_escalations = sum(1 for r in results if r.policy_decision_status == "ESCALATE" and not r.policy_matched)
        ...
        return EvaluationMetrics(
            ...
            diagnosis_accuracy=sum(1 for r in results if r.diagnosis_matched) / total if total else 0,
            recommendation_accuracy=sum(1 for r in results if r.action_matched) / total if total else 0,
            policy_decision_accuracy=sum(1 for r in results if r.policy_matched) / total if total else 0,
